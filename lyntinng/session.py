...
class Session:
   ...
   def getWriteFileInfo(self):
      ...
      data += self._aliasmanager.getAliasInfo() + "\n"
      data += self._actionmanager.getActionInfo() + "\n"
      data += self._gagmanager.getGagInfo() + "\n"
      data += self._submanager.getSubstituteInfo() + "\n"
      data += self._varmanager.getVariableInfo() + "\n"
      return data
   ...
   def handleUserData(self, input, internal=0):
      ...
      if lyntin.speedwalk == 1:
         ...
         if SPEEDWALK_REGEXP.search(input) and input != 'news':
            self._socket.write(self.expandSpeedwalk(input))
            return
